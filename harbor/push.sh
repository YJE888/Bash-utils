...
# #harbor-domain_입력 부분에 host명 입력 ex) test-repo.com
if [ $1 = "-h" ] || [ $1 = "--help" ]
then
...
ssh ${NODE} "docker commit ${CID} #harbor-domain_입력/${NS}/${REPO}:${TAG};cat /root/repo/my_password.txt | docker login #harbor-domain_입력 --username admin --password-stdin;docker push # harbor-domain_입력/${NS}/${REPO}:${TAG};docker logout # harbor-domain_입력;exit;"
